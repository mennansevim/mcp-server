...
class ReviewStagedChangesAction : AnAction() {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        ProgressManager.getInstance().run(object : Task.Backgroundable(project, "MCP AI Code Review - Staged Changes", true) {
            override fun run(indicator: ProgressIndicator) {
                indicator.text = "Getting staged changes..."
                indicator.fraction = 0.2
                
                val diff = gitService.getStagedDiff()
                val files = gitService.getStagedFiles()
                
                if (diff.isBlank()) {
                    showNotification(project, "No staged changes found", NotificationType.WARNING)
                    return
                }
                
                indicator.text = "Reviewing ${files.size} staged file(s)..."
                ...
                mcpClient.reviewViaWebhook(diff, files) { result ->
                    result.fold(
                        onSuccess = { response ->
                            ...
                            showNotification(project, "Review completed for ${files.size} staged file(s)", NotificationType.INFORMATION)
