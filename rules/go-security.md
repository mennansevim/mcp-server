...
**Bad:**
```go
cmd := exec.Command("ping", user_input)
```

**Good:**
```go
cmd := exec.Command("ping", "-c", "1", user_input)
```
...
### Random Numbers
- ❌ **HIGH:** Don't use `math/rand` for security-sensitive operations
...
- ❌ **HIGH:** Don't share variables between goroutines without synchronization

### Error Handling
- ✅ **ALWAYS** check errors
- ❌ **MEDIUM:** Don't ignore errors
